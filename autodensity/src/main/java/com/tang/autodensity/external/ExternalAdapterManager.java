...
/**
 * ================================================
 * 管理三方库的适配信息和状态, 通过 {@link com.tang.autodensity.DensityConfig#getExternalAdaptManager()} 获取, 切勿自己 new
...
public class ExternalAdapterManager {
    ...
    private Map<String, ExternalAdapterInfo> mExternalAdaptInfos;
     ...
     * <p>
     * 支持链式调用, 如:
     * {@link ExternalAdapterManager#addCancelAdaptOfActivity(Class)#addCancelAdaptOfActivity(Class)}
     *
     * @param targetClass {@link Activity} class, {@link Fragment} class
     */
    public synchronized ExternalAdapterManager addCancelAdaptOfActivity(Class<?> targetClass) {
     ...
     * <p>
     * 支持链式调用, 如:
     * {@link ExternalAdapterManager#addExternalAdaptInfoOfActivity(Class, ExternalAdapterInfo)#addExternalAdaptInfoOfActivity(Class, ExternalAdapterInfo)}
     *
     * @param targetClass {@link Activity} class, {@link Fragment} class
     * @param info        {@link ExternalAdapterInfo} 适配参数
     */
    public synchronized ExternalAdapterManager addExternalAdaptInfoOfActivity(Class<?> targetClass, ExternalAdapterInfo info) {
        Preconditions.checkNotNull(targetClass, "targetClass == null");
        if (!isRun) {
            isRun = true;
        }
     ...
     */
    public synchronized ExternalAdapterInfo getExternalAdaptInfoOfActivity(Class<?> targetClass) {
        Preconditions.checkNotNull(targetClass, "targetClass == null");
        if (mExternalAdaptInfos == null) {
            return null;
        }
